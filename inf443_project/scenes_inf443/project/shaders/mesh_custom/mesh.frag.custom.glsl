...
void main()
{
	...
	//DEgradé
	vec3 colorB = vec3(0.08, 0.35, 0.13);
	vec3 colorA = vec3(0.85, 0.71, 0.25);
    vec2 st = vec2(fragment.position.y,fragment.position.z);
	vec3 color = vec3(0.0);
	...
	//pct.g = pow(st.y,5.0);
	//pct.b = pow(st.y,0.5);
	pct.r = smoothstep(0.1,0.45,st.y);
	pct.g = smoothstep(0.1,0.45,st.y);
	pct.r = smoothstep(0.1,0.45,st.y);
	...
	//FragColor = abs(vec4(0, cos(fragment.position.z), 0.0, 0.0));
